...
def conv(layer_name, x, out_channels, kernel_size=[3,3], stride=[1,1,1,1], is_pretrain=True):
    ...
    with tf.variable_scope(layer_name):
        w = tf.get_variable(name='weights',
                            trainable=is_pretrain,  # true：从已训练好的数据里赋值 false：随机初始化
                            shape=[kernel_size[0], kernel_size[1], in_channels, out_channels],
                            initializer=tf.contrib.layers.xavier_initializer()) # xavier这个初始化源自一篇论文
...
def batch_norm(x):
    epsilon = 1e-3
    batch_mean, batch_var = tf.nn.moments(x, [0]) 	# 计算x的均值和方差
    x = tf.nn.batch_normalization(x,    			# 批量改变x空间上的分布，源于论文，很利于收敛
                                  mean=batch_mean,
                                  variance=batch_var,
                                  offset=None,
                                  scale=None,
...
def test_load():
    data_path = './/vgg16_pretrain//vgg16.npy'  #加载已经训练好的参数
